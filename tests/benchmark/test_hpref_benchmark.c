...
static struct hpref_node *test_pointer;
...
void *thr_reader(void *_count)
{
	...
	for (;;) {
		...
		res = hpref_hp_get(&test_pointer, &ctx);
		rcu_debug_yield_read();
		if (res) {
			struct test *test = caa_container_of(hpref_ctx_pointer(&ctx), struct test, node);
			urcu_posix_assert(test->value == 8);
...
{
	struct test *test = caa_container_of(node, struct test, node);
...
void *thr_writer(void *_count)
{
	...
	for (;;) {
		...
		hpref_node_init(&new->node, test_release);
		new->value = 8;
		old_node = rcu_xchg_pointer(&test_pointer, &new->node);
		if (caa_unlikely(wduration))
			loop_sleep(wduration);
		hpref_synchronize_put(old_node);
		URCU_TLS(nr_writes)++;
	...
	old_node = rcu_xchg_pointer(&test_pointer, NULL);
	hpref_synchronize_put(old_node);
